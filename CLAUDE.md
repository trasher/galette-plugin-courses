...
- Phase 32 : TERMINEE - Acces "Mes seances comme moniteur" elargi : la condition d'affichage du menu et de la tuile dashboard passe de `countSessionsForMember > 0` a `isAdmin() || isStaff() || isGroupManager() || (countSessionsForMember > 0)` dans `PluginGaletteCourses::getMenusContents()` et `getDashboardsContents()` ; un responsable de groupe sans seance assignee voit maintenant l'entree pour se proposer volontaire via l'onglet *Trouver une seance*
