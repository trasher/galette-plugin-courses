...
- Phase 31 : TERMINEE - Filtres "Trouver une seance" : abandon des dropdowns Fomantic UI (synchronisation `<select>.val()` non fiable, `change` natif pas declenche) au profit de `<select>` HTML natifs (`class="courses-native-select"`) sur Mes inscriptions et Mes seances comme moniteur ; ajout d'un bouton **Filtrer** explicite (`#browse_apply_filter` / `#instr_browse_apply_filter`) en plus du bouton Effacer ; lecture des data attributes via `.attr()` plutot que `.data()` (bypass cache jQuery) ; comparaison Activite trim+lowercase ; nouvelles classes CSS `.courses-native-select` (style coherent avec `.ui.input`) et `.courses-filter-actions` (flex container, boutons pleine largeur sur mobile)
